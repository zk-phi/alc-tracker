...
function getChart (numRows) {
    var tbl = Charts.newDataTable()
                    ...
                    .addColumn(Charts.ColumnType.NUMBER, 'consumed')
                    .addColumn(Charts.ColumnType.NUMBER, 'pacemaker')
                    .addColumn(Charts.ColumnType.NUMBER, 'base')
                    .addColumn(Charts.ColumnType.NUMBER, 'limit');

    var sheet = _getSheet();
    var data = sheet.getRange(2, 1, numRows + 1, 5).getValues();
    data.forEach(function (row) { tbl.addRow(row); });
    tbl.build();
